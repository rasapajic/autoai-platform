...
function getSerbiaEligibility(listing: any) {
  ...
  if (year) {
    ...
    if (year >= 2001) return { status: 'needs_check', emoji: '🟠', label: 'Potrebna dodatna provera', reason: `Vozilo (${year}) je verovatno Euro 3 — uvoz zahteva proveru.`, warnings: ['Proveri Euro normu u COC dokumentu.'], carinaPct: 5 }
    return { status: 'not_recommended', emoji: '🔴', label: 'Verovatno nije moguće', reason: `Vozilo (${year}) je verovatno Euro 1/2 — uvoz nije preporučljiv.`, warnings: ['Stara vozila teško prolaze tehnički pregled.'], carinaPct: 5 }
  }

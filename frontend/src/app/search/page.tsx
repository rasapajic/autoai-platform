...
function getSerbiaEligibility(listing: any) {
  ...
  if (!year) return {
    ...
    reason: fuel === 'diesel' ? 'Dizel vozila zahtevaju posebnu proveru DPF filtera i Euro norme.'
          : fuel === 'petrol' ? 'Benzinska vozila — potrebno proveriti Euro normu.'
  ...
  if (year >= 2006) return {
    ...
    warnings: ['Euro 4 je granica — obavezno pribavi COC dokument pre kupovine.', ...(fuel === 'diesel' ? ['Dizel Euro 4 — proveri DPF i turbinu.'] : []), 'Bez COC dokumenta može biti problem pri tehničkom pregledu.'],
    carinaPct: 5,
  }
  ...
  if (year >= 2001) return {
    ...
    tooltip: 'Kupovina je moguća, ali preporučujemo dodatnu proveru pre uvoza u Srbiju.',
    warnings: ['Euro 3 vozila mogu imati poteškoće pri tehničkom pregledu u Srbiji.', 'Obavezno proveri Euro normu u COC dokumentu pre kupovine.', 'Konsultuj carinskog agenta ili MUP Srbije pre uvoza.'],
    carinaPct: 5,
  }
  ...
  if (year >= 1997) return {
    ...
    warnings: ['Euro 1/2 vozila teško prolaze tehnički pregled u Srbiji.', 'Razmotri oldtimer status ako je vozilo starije od 30 godina.'],
    carinaPct: 5,
  }
  ...
  useEffect(() => {
    Promise.allSettled([
      ...
      if (l.status === 'fulfilled') {
        const data = l.value
        setListing(data)
        // Auto-enrich ako nema godišta ili km
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 340px', gap: 28, alignItems: 'start' }}>
          ...
          <div>
            ...
            <div style={{ background:'var(--bg2)', border:'1px solid var(--border)', borderRadius:'var(--radius)', padding:24, marginBottom:20 }}>
              ...
              {enriching && specs.length === 0 ? (
                ...
                <p style={{ color:'var(--text3)', fontSize:13 }}>Specifikacije se učitavaju...</p>
              )}
            </div>
          ...
          <div style={{ position:'sticky', top:80, display:'flex', flexDirection:'column', gap:14 }}>
            ...
            <div style={{ background:'var(--bg2)', border:'1px solid var(--border)', borderRadius:'var(--radius)', padding:22 }}>
              ...
              {/* Sekundarno refresh dugme */}
              <button onClick={manualRefresh} disabled={enriching} style={{
                width:'100%', padding:'8px', marginBottom:8,
                background:'transparent',
                border:'1px solid var(--border)',
                color: enriching ? 'var(--text3)' : 'var(--text3)',
                borderRadius:10, fontSize:12, cursor: enriching ? 'default' : 'pointer',
                opacity: enriching ? 0.5 : 0.7,
              }}>
                {enriching ? '⏳ AI analizira...' : enriched ? '✅ Podaci ažurirani' : '🔄 Ažuriraj podatke sa portala'}
              </button>

        ...
        <div style={{
          ...
          <div style={{ flex: 1, minWidth: 180 }}>
            <p className="ht" style={{
              fontSize: 21, fontWeight: 700, margin: 0,
              fontFamily: 'Syne,sans-serif', lineHeight: 1.3,
            }}>Ne kupuj auto iz EU naslepo</p>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20, flexWrap: 'wrap', gap: 10 }}>
              <div>
                ...
                {!loading && results?.total > 0 && (
                  <span style={{ color: 'var(--text3)', fontSize: 13, marginLeft: 8 }}>analiziranih AI-om</span>
                )}
              </div>
...
function ListingCard({ listing }: { listing: any }) {
  ...
  return (
    <div className="ch" style={{
      ...
      {/* ── AI Badge — top of card, full width ── */}
      {badge ? (
        <div style={{
          ...
          <span style={{ color: badge.color, fontSize: 13, fontWeight: 800, letterSpacing: '.03em' }}>
            {badge.label}
          </span>
          {delta !== null && (
        ...
        <div style={{
          background: 'rgba(99,102,241,.07)', borderBottom: '2px solid rgba(99,102,241,.2)',
          padding: '9px 16px',
      ...
      <a href={`/listing/${listing.id}`} style={{ display: 'block', textDecoration: 'none' }}>
        ...
        <div style={{ padding: '16px 18px 0' }}>
          <h3 style={{
            ...
            fontFamily: 'Syne,sans-serif', lineHeight: 1.3, color: 'var(--text)',
          }}>
          ...
          {insight && (
            <p style={{
              fontSize: 12, color: badge?.color || '#818CF8',
              margin: '0 0 12px', display: 'flex', alignItems: 'center', gap: 5,
            }}>
              <span style={{
                width: 5, height: 5, borderRadius: '50%',
                background: badge?.color || '#818CF8',
        ...
        {bd && (
          <div style={{
            ...
            >
              ...
              <span style={{ fontSize: 11, color: 'rgba(255,107,0,.6)', userSelect: 'none' }}>
                {showBreakdown ? '▲ sakrij' : '▼ detalji'}
              </span>
            </button>
            ...
            {showBreakdown && (
              <div style={{
                padding: '0 14px 12px',
                borderTop: '1px solid rgba(255,107,0,.15)',
              }}>
                <div style={{ fontSize: 11, color: 'var(--text3)', marginBottom: 8, marginTop: 8 }}>
                  Kako je izračunato:
                </div>
                {[
                  ...
                  <div key={i} style={{
                    ...
                    <span style={{ fontSize: 12, color: i === 0 ? 'var(--text2)' : 'var(--text3)' }}>
                      {i > 0 && '+ '}{label}
                      {note && <span style={{ fontSize: 10, marginLeft: 4, opacity: .55 }}>({note})</span>}
                    </span>
                    <span style={{
                      fontSize: 12, fontWeight: i === 0 ? 400 : 500,
                      color: i === 0 ? 'var(--text2)' : '#fb923c',
                    }}>
                ...
                <div style={{
                  ...
                  borderTop: '1px solid rgba(255,107,0,.25)',
                }}>
                  <span style={{ fontSize: 13, fontWeight: 700, color: 'var(--text)' }}>Ukupno Srbija</span>
      ...
      <FS label="AI Ocjena">
        <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
          {Object.entries(AI_BADGES).map(([v,{label,color}]) => (
            <FC key={v} label={label} active={filters.price_rating===v}
              onClick={()=>setFilter('price_rating',filters.price_rating===v?'':v)}
              color={color} full />
          ))}
        </div>
      </FS>
      <button className="cb" onClick={onReset} style={{
...
function FC({ label, active, onClick, color, full }: any) {
  return (
    <button className="cb" onClick={onClick} style={{
      ...
      padding: full ? '7px 12px' : '5px 11px',

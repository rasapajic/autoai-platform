...
def _parse_listing(item: dict) -> dict | None:
    try:
        ...
        make      = _get_attr(all_attrs, "merk", "make", "brand")
        model     = _get_attr(all_attrs, "model")
        year_str  = _get_attr(all_attrs, "constructionyear", "bouwjaar", "year")
        km_str    = _get_attr(all_attrs, "mileage", "kilometerstand", "km")
        fuel_str  = _get_attr(all_attrs, "fuel", "brandstof")
        trans_str = _get_attr(all_attrs, "transmission", "transmissie")
        body_str  = _get_attr(all_attrs, "body", "carrosserie")
        power_str = _get_attr(all_attrs, "power", "vermogen")
        color_str = _get_attr(all_attrs, "color", "kleur")
        ...
        if vip_url and not vip_url.startswith("http"):
            vip_url = f"https://www.2dehands.be{vip_url}"
        if not vip_url:
            vip_url = f"https://www.2dehands.be/a/{item_id}.html"

        return {
            "external_id":     f"tdh_{item_id}",
            "source":          "2dehands",
        ...
        print(f"[2dehands] Parse greška: {e}")
        return None
    ...
    async def scrape_listings(self, filters: dict, max_pages: int = 10) -> list:
        ...
        async with aiohttp.ClientSession(headers=HEADERS, trust_env=True) as session:
            for page_num in range(max_pages):
                ...
                print(f"[2dehands] Stranica {page_num + 1}, offset={offset}")

                try:
                    async with session.get(API_URL, params=params,
                                           timeout=aiohttp.ClientTimeout(total=20)) as resp:
                        print(f"[2dehands] Status: {resp.status}")
                        if resp.status != 200:
                            break
                        ...
                        print(f"[2dehands] listings={len(regular)}, topBlock={len(top)}, total={data.get('totalResultCount', 0)}")
                        ...
                        print(f"[2dehands] Str.{page_num+1}: +{added} | Ukupno: {len(all_listings)}")
                    ...
                    print(f"[2dehands] Greška str.{page_num+1}: {e}")
                    break

        print(f"[2dehands] Završeno — {len(all_listings)} oglasa")
        return all_listings

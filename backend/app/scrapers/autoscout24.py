...
async def run_autoscout24():
    ...
    try:
        ...
        # Scrape po tipu goriva — svaki dobija jednaku zastupljenost
        fuel_types = [
            ("diesel",   "D", 15),
            ("petrol",   "B", 15),
            ("electric", "E", 10),
        ...
        for fuel_name, fuel_code, pages in fuel_types:
            print(f"  AutoScout24 DE — {fuel_name} ({pages} stranica)...")
...
async def run_autoscout24_at():
    ...
    try:
        ...
        for fuel_name, fuel_code, pages in fuel_types:
            print(f"  AutoScout24 AT — {fuel_name} ({pages} stranica)...")
            scraper  = AutoScout24Scraper()
            listings = await scraper.scrape_listings(
